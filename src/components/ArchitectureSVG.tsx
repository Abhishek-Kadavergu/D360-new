  ...
  const elements = useMemo(() => {
    ...
    for (let i = -14; i <= 14; i++) {
       els.push(
         <line key={`gv-${i}`} x1={cx + i * 55} y1={830} x2={cx + i * 12} y2={180}
           stroke={gridColor} strokeWidth={1} opacity={gridOp * 0.8} />
       );
    }
    for (let i = 0; i < 16; i++) {
       const y = 830 - i * 42;
       ...
       els.push(
         <line key={`gh-${i}`} x1={cx - 420 * s} y1={y} x2={cx + 420 * s} y2={y}
           stroke={gridColor} strokeWidth={0.8} opacity={gridOp * 0.7 * (1 - i * 0.05)} />
       );
    }
    ...
    if (z1 > 0) {
      ...
      els.push(
        <g key="hopper" opacity={z1}>
          <path d="M 250 785 L 170 725 L 170 695 L 250 730 Z" fill={shapeFill} opacity={0.25} stroke={shapeStroke} strokeWidth={2} />
          <path d="M 250 785 L 750 785 L 830 725 L 170 725 Z" fill={shapeFill} opacity={0.15} stroke={shapeStroke} strokeWidth={1.5} />
          <path d="M 750 785 L 830 725 L 830 695 L 750 730 Z" fill={shapeFill} opacity={0.3} stroke={shapeStroke} strokeWidth={2} />
          <path d="M 250 730 L 750 730 L 830 695 L 170 695 Z" fill={shapeFill} opacity={0.12} stroke={shapeStroke} strokeWidth={1.5} />
          <text x={cx} y={762} textAnchor="middle" fill={textMain} fontSize="17" fontFamily="monospace" fontWeight="900" opacity={0.95}>
            ◆ RAW DATA INPUT ◆
          </text>
        </g>
      );
      ...
      types.forEach((d, i) => {
        ...
        els.push(
          <g key={`dt-${i}`} opacity={z1 * 0.9}>
            <text x={d.x} y={770 + bob} textAnchor="middle" fontSize="24">{d.icon}</text>
            <text x={d.x} y={794} textAnchor="middle" fill={textMain} fontSize="13" fontFamily="monospace" fontWeight="800" opacity={0.95}>{d.label}</text>
          </g>
        );
      });
      ...
      for (let i = 0; i < 25; i++) {
        ...
        els.push(
          <circle key={`dp-${i}`} cx={px} cy={py} r={2 + Math.random()}
            fill={shapeStroke} opacity={z1 * 0.65 * (1 - t)}>
            <animate attributeName="opacity" values="0.8;0.3;0.8" dur={`${0.6 + i * 0.08}s`} repeatCount="indefinite" />
          </circle>
        );
      }
      ...
      els.push(
        <g key="in-arrow" opacity={z1 * 0.8}>
          <polygon points={`${cx},678 ${cx - 8},698 ${cx + 8},698`} fill={shapeStroke} opacity={0.85} />
        </g>
      );
    }
    ...
    if (z2 > 0) {
      els.push(
        <g key="conduit" opacity={z2}>
          <rect x={cx - 70} y={635} width={140} height={50} rx={6} fill={shapeFill} opacity={0.25} stroke={shapeStroke} strokeWidth={2.5} />
          <path d={`M ${cx - 70} 635 L ${cx - 58} 623 L ${cx + 82} 623 L ${cx + 70} 635 Z`} fill={shapeFill} opacity={0.3} stroke={shapeStroke} strokeWidth={2} />
          <path d={`M ${cx + 70} 635 L ${cx + 82} 623 L ${cx + 82} 673 L ${cx + 70} 685 Z`} fill={shapeFill} opacity={0.35} stroke={shapeStroke} strokeWidth={2} />
          {Array.from({ length: 8 }).map((_, i) => {
            const bx = cx - 58 + ((p * 250 + i * 18) % 116);
            return (
              <rect key={`cd-${i}`} x={bx} y={645} width={3} height={30} rx={1}
                fill={shapeStroke} opacity={z2 * 0.8}>
                <animate attributeName="opacity" values="0.9;0.3;0.9" dur={`${0.4 + i * 0.08}s`} repeatCount="indefinite" />
              </rect>
            );
          })}
          <text x={cx} y={665} textAnchor="middle" fill={textMain} fontSize="14" fontFamily="monospace" fontWeight="900" opacity={0.95}>
            TOKENIZER + EMBED
          </text>
        </g>
      );
    ...
    if (z3 > 0) {
      ...
      els.push(
        <g key="tbox" opacity={z3 * 0.95}>
          <rect x={bL} y={bT} width={bR - bL} height={bB - bT} rx={4}
            fill={shapeFill} opacity={0.15} stroke={shapeStroke} strokeWidth={3} />
          <path d={`M ${bL} ${bT} L ${bL + dep} ${bT - dep} L ${bR + dep} ${bT - dep} L ${bR} ${bT} Z`}
            fill={shapeFill} opacity={0.25} stroke={shapeStroke} strokeWidth={2} />
          <path d={`M ${bR} ${bT} L ${bR + dep} ${bT - dep} L ${bR + dep} ${bB - dep} L ${bR} ${bB} Z`}
            fill={shapeFill} opacity={0.3} stroke={shapeStroke} strokeWidth={2} />
          <text x={cx + 22} y={bT - dep / 2 + 6} textAnchor="middle" fill={textMain} fontSize="21"
      ...
      layers.forEach((layer, i) => {
          ...
          for (let j = 0; j < Math.min(layer.n, 5); j++) {
            ...
            els.push(
              <line key={`tc-${i}-${j}`}
                ...
                stroke={layer.c} strokeWidth={ignited ? 2 : 1.2} opacity={lop * (ignited ? 0.6 : 0.35)} />
            );
          }
        }
        ...
        for (let j = 0; j < layer.n; j++) {
          ...
          els.push(
            <g key={`tn-${i}-${j}`} opacity={lop}>
              {ignited && (
                <circle cx={nx} cy={layer.y} r={12} fill={layer.c} opacity={0.25}>
                  <animate attributeName="r" values="8;14;8" dur={`${1.8 + j * 0.15}s`} repeatCount="indefinite" />
                </circle>
              )}
              <circle cx={nx} cy={layer.y} r={4.5} fill={layer.c} opacity={ignited ? 0.9 : 0.6} />
              <circle cx={nx} cy={layer.y} r={1.8} fill="#fff" opacity={ignited ? 1 : 0.6} />
            </g>
          );
        }
        ...
        els.push(
          <g key={`tl-${i}`} opacity={lop * 1}>
            <line x1={bR + 8} y1={layer.y - 8} x2={bR + 8} y2={layer.y + 8} stroke={dimBlue} strokeWidth={1.5} />
            <line x1={bR + 8} y1={layer.y} x2={bR + 48} y2={layer.y} stroke={dimBlue} strokeWidth={1.5} />
            <text x={bR + 56} y={layer.y + 4} fill={textSub} fontSize="14" fontWeight="800" fontFamily="monospace">{layer.label}</text>
          </g>
        );
      });
      ...
      if (z3 > 0.25) {
        for (let i = 0; i < 15; i++) {
          ...
          els.push(
            <circle key={`tf-${i}`} cx={px} cy={550} r={2.5} fill={shapeStroke} opacity={z3 * 0.8}>
              <animate attributeName="cy" values="590;310" dur={`${2.5 + i * 0.25}s`} repeatCount="indefinite" />
              <animate attributeName="opacity" values="0.8;0.2;0.8" dur={`${2.5 + i * 0.25}s`} repeatCount="indefinite" />
            </circle>
          );
        }
      }
      ...
      if (z3 > 0.15) {
        ...
        // Left gear
        els.push(
          <g key="gear-l" opacity={z3 * 0.6}
            style={{ transform: `rotate(${rot}deg)`, transformOrigin: `${bL - 40}px 460px`, transition: "transform 1s linear" }}>
            <path d={gearPath(bL - 40, 460, 28, 10)} fill="none" stroke={shapeStroke} strokeWidth={2.5} />
            <circle cx={bL - 40} cy={460} r={7} fill="none" stroke={shapeStroke} strokeWidth={2} />
          </g>
        );
        // Right gear
        els.push(
          <g key="gear-r" opacity={z3 * 0.6}
            style={{ transform: `rotate(${-rot}deg)`, transformOrigin: `${bR + 80}px 490px`, transition: "transform 1s linear" }}>
            <path d={gearPath(bR + 80, 490, 24, 9)} fill="none" stroke={shapeStroke} strokeWidth={2.5} />
            <circle cx={bR + 80} cy={490} r={6} fill="none" stroke={shapeStroke} strokeWidth={2} />
          </g>
        );
      }
          ...
          els.push(
            <g key="spark" opacity={0.9}>
              ...
              <circle cx={cx} cy={active.y} r={22} fill={amber} opacity={0.35}>
                <animate attributeName="r" values="12;26;12" dur="0.5s" repeatCount="indefinite" />
              </circle>
              {[0, 72, 144, 216, 288].map((a) => {
                ...
                return (
                  <line key={`sp-${a}`}
                    ...
                    x2={cx + Math.cos(rad) * 16} y2={active.y + Math.sin(rad) * 16}
                    stroke={amber} strokeWidth={2} opacity={0.8}>
                    <animate attributeName="opacity" values="0.9;0;0.9" dur="0.2s" begin={`${a * 0.005}s`} repeatCount="indefinite" />
                  </line>
                );
              })}
            </g>
      ...
      els.push(
        <g key="out-funnel" opacity={z4}>
          <path d={`M ${cx - 55} 295 L ${cx - 130} 190 L ${cx + 130} 190 L ${cx + 55} 295 Z`}
            fill={shapeFill} opacity={0.2} stroke={shapeStroke} strokeWidth={2.5} />
          <path d={`M ${cx + 55} 295 L ${cx + 130} 190 L ${cx + 150} 200 L ${cx + 75} 300 Z`}
            fill={shapeFill} opacity={0.3} stroke={shapeStroke} strokeWidth={2} />
          <text x={cx} y={250} textAnchor="middle" fill={textMain} fontSize="17" fontFamily="monospace" fontWeight="900" opacity={0.95}>
            OUTPUT PROJECTION
          </text>
        </g>
      );
        ...
        els.push(
          <g key={`out-${i}`} opacity={oOp}>
            <line x1={cx} y1={200} x2={out.x} y2={out.y + 22}
              stroke={shapeStroke} strokeWidth={ignited ? 3 : 2} opacity={0.6}
              strokeDasharray={ignited ? "none" : "5 3"} />
            <circle cx={out.x} cy={out.y + 22} r={ignited ? 26 : 22} fill="none"
              stroke={shapeStroke} strokeWidth={1.5} opacity={ignited ? 0.7 : 0.5} />
            <circle cx={out.x} cy={out.y + 22} r={ignited ? 28 : 24} fill="none"
              stroke={shapeStroke} strokeWidth={3} opacity={ignited ? 0.25 : 0.12} />
            <text x={out.x} y={out.y + 17} textAnchor="middle" fontSize="24" opacity={0.95}>{out.icon}</text>
            <text x={out.x} y={out.y + 44} textAnchor="middle" fill={textMain} fontSize="13" fontFamily="monospace" fontWeight="800" opacity={0.95}>
              {out.label}
            </text>
            {ignited && (
              <circle cx={out.x} cy={out.y + 22} r={4} fill={shapeStroke} opacity={0.8}>
                <animate attributeName="cy" values={`${out.y + 22};${out.y - 10}`} dur={`${1.2 + i * 0.25}s`} repeatCount="indefinite" />
                <animate attributeName="opacity" values="0.8;0;0.8" dur={`${1.2 + i * 0.25}s`} repeatCount="indefinite" />
              </circle>
            )}
          </g>
        );
      ...
      if (z4 > 0.4) {
        els.push(
          <g key="ai-badge" opacity={z4}>
            <circle cx={cx} cy={48} r={36} fill={shapeFill} opacity={ignited ? 0.35 : 0.2} stroke={shapeStroke} strokeWidth={3} />
            <text x={cx} y={58} textAnchor="middle" fill={textMain} fontSize="32" fontFamily="monospace" fontWeight="900" opacity={0.95}>
              AI
            </text>
            {ignited && (
              <circle cx={cx} cy={48} r={42} fill="none" stroke={shapeStroke} strokeWidth={2} opacity={0.6}>
    ...
    if (p > 0.08) {
      els.push(
        <g key="fa1" opacity={aOp}>
          <line x1={cx} y1={695} x2={cx} y2={637} stroke={shapeStroke} strokeWidth={2.5} strokeDasharray="6 3">
            <animate attributeName="strokeDashoffset" values="0;-18" dur="0.8s" repeatCount="indefinite" />
          </line>
          <polygon points={`${cx},635 ${cx - 6},649 ${cx + 6},649`} fill={shapeStroke} opacity={0.9} />
        </g>
      );
    }
    if (p > 0.55) {
      els.push(
        <g key="fa2" opacity={aOp}>
          <line x1={cx} y1={300} x2={cx} y2={296} stroke={shapeStroke} strokeWidth={2.5} strokeDasharray="6 3">
            <animate attributeName="strokeDashoffset" values="0;-18" dur="0.8s" repeatCount="indefinite" />
          </line>
          <polygon points={`${cx},293 ${cx - 6},307 ${cx + 6},307`} fill={shapeStroke} opacity={0.9} />
        </g>
      );
    }
    ...
    if (ignited) {
      for (let i = 0; i < 6; i++) {
        ...
        els.push(
          <circle key={`en-${i}`} cx={px} cy={700} r={3} fill={shapeStroke} opacity={0.75}>
            <animate attributeName="cy" values="700;80" dur={`${3 + i * 0.5}s`} repeatCount="indefinite" />
            <animate attributeName="opacity" values="0.75;0.1;0.75" dur={`${3 + i * 0.5}s`} repeatCount="indefinite" />
          </circle>
        );
      }
    }
    ...
    >
      <defs>
        <radialGradient id="core-glow" cx="50%" cy="55%" r="45%">
          <stop offset="0%" stopColor="#87cefa" stopOpacity={ignited ? 0.45 : 0.15} />
      ...
      {ignited && (
        <text x={500} y={838} textAnchor="middle" fill="#0B2039" fontSize="14" fontFamily="monospace" fontWeight="800" letterSpacing="3" opacity={0.9}>
